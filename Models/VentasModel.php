...
class VentasModel extends Mysql{
    ...
    {
        $sql = "SELECT v.id as id, v.total as total, v.fecha as fecha ,v.id_cliente as id_cliente ,v.estado as estado, c.nombre as nombre   FROM ventas  as v INNER JOIN clientes  as c ON v.id_cliente = c.id WHERE v.estado = 1 ";
    ...
    public function selectVentasFecha(string $ini, string $hasta)
    {
        //$sql = "SELECT v.id as id, v.total as total, v.fecha as fecha ,v.id_cliente as id_cliente ,v.estado as estado, c.nombre as nombre   FROM ventas  as v INNER JOIN clientes  as c ON v.id_cliente = c.id WHERE v.fecha BETWEEN '$ini' AND '$hasta'";
        $sql = "SELECT v.id as id, v.total as total, v.fecha as fecha ,v.id_cliente as id_cliente ,v.estado as estado, c.nombre as nombre   FROM ventas  as v INNER JOIN clientes  as c ON v.id_cliente = c.id WHERE v.fecha >= '$ini' AND v.fecha <='$hasta'";
        $res = $this->select_all($sql);
        return $res;
    }
    ...
    {
        $this->codigo = $codigo;
        $sql = "SELECT * FROM productos WHERE codigo = '$this->codigo' OR nombre = '$this->codigo' AND estado = 1";
        $res = $this->select($sql);
        if (empty($res)) {
            $res = 0;
        }
    ...
    {
        $sql = "SELECT v.id_venta as idventa, v.fecha as fecha, u.nombre as usuario ,cantidad , precio ,producto FROM detalle_venta  as v INNER JOIN usuarios  as u ON v.id_usuario=u.id WHERE v.id_venta = $id_venta";
        $res = $this->select_all($sql);
        return $res;
    }
